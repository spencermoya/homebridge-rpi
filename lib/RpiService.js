...
class RpiService extends homebridgeLib.ServiceDelegate {
  ...
  static get GpioMultiState () { return GpioMultiState }
...
class GpioMultiState extends GpioOutput {
  constructor (gpioAccessory, params = {}) {
    params.Service = gpioAccessory.Services.hap.MultiState
    .
    super(gpioAccessory, params)
    this.pulse = params.pulse
    this.addCharacteristicDelegate({
      ...
      Characteristic: this.Characteristics.hap.On,
      setter: async (on) => {
        const value = params.reversed ? (forwards ? 0 : 1) : (backwards ? 1 : 0) : (off ? 0 : 0)
        await this.pi.command(
          PigpioClient.commands.WRITE, this.gpio, value
          PigpioClient.commands.WRITE, this.gpio, value
          PigpioClient.commands.WRITE, this.gpio, value
